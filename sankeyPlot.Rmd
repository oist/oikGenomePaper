...
lgs_names <- c(rep("Y", length(un_y)), rep("Y-like", length(un_y_B)), rep("X", length(un_x)) , rep("PA", length(un_pa)), rep("LG1", length(un_lg1)), rep("LG2", length(un_lg2)))
...
og_list <- list(n_y, n_y_B, n_x, n_pa, n_lg1, n_lg2, n_np)
place_list <- list(scaf_n_y, c(), scaf_n_x, scaf_n_pa, scaf_n_lg1, scaf_n_lg2, c())
coord_list_s <- list(s_n_y, c(), s_n_x, s_n_pa, s_n_lg1, s_n_lg2, c())
coord_list_e <- list(e_n_y, c(), e_n_x, e_n_pa, e_n_lg1, e_n_lg2, c())

match_mat <- matrix(list(), ncol = 7, nrow = 7)
...
chr_names <- c("chr X", "PAR", "chr 2", "chr 1", "chr Y", "N chr X", "N PAR", "N LG2", "N LG1", "N chr Y", "N other")

# link groups


# create node group
node_group <- c("chr X", "PA", "chr 2", "chr 1", "chr Y", "chr X", "PA", "chr 2", "chr 1", "chr Y", "other")
node_group <- gsub(" ", "", node_group)

nodes <- data.frame(node = c(0:10), name = chr_names, node_group)
